...
if model.status == GRB.Status.OPTIMAL: # If optimal solution is found
    ...
    total_production = sum((x[i, k].X * CostSup[i]) for i in I for k in K)  # Calculate total sum of production
    print(f'Total buying costs across all products and months: {total_production:.2f} euro.')
